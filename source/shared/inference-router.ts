export const SAND_INFERENCE_PROVIDERS = ["cursor", "claude-code", "codex", "openrouter", "cli-proxy"] as const;
...
export function emptySandInferenceRouterUsage(): SandInferenceRouterUsage {
  ...
  return { schemaVersion: 1, providers: { cursor: empty(), "claude-code": empty(), codex: empty(), openrouter: empty(), "cli-proxy": empty() } };
}
