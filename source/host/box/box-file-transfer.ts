...
export async function runBoxShell(ctx: Context, accessor: FileTransferAccessor, script: string): Promise<void> { const command = `bash -lc ${shellSingleQuote(script)}`; const result = (await accessor.get(shellExecutorResource).execute(ctx, buildHostShellArgs({ command, name: "bash", workingDirectory: "/workspace", toolCallId: TRANSFER_TOOL_CALL_ID }))).result; if (result.case === "success" && result.value.exitCode === 0) return; const transient = asTransientBoxShellError(result, "box shell command"); if (transient) throw transient; throw new SandBoxFileTransferError(`box shell command failed (${describeShellFailure(result)})`); }
...
export async function uploadFileViaExecDaemon(
  ctx: Context,
  accessor: FileTransferAccessor,
  boxPath: string,
  data: Uint8Array,
): Promise<void> {
  const part = `${boxPath}.sand-${randomBytes(8).toString("hex")}.part`;
  try {
    await writeFileBytesViaExecDaemon(ctx, accessor, part, data);
    await runBoxShell(
      ctx,
      accessor,
      `mv -f -- ${shellSingleQuote(part)} ${shellSingleQuote(boxPath)}`,
    );
  } finally {
    // A write or mv acknowledgement can be lost after bytes materialize. The
    // randomized staging file must be removed on both failure paths.
    await runBoxShell(
      ctx,
      accessor,
      `rm -f -- ${shellSingleQuote(part)}`,
    ).catch(() => undefined);
  }
}
