...
test("Windows packaged smoke exercises the complete signed-out 9Router setup in dependency order", async () => {
  ...
  assert.match(source, /Persistent relaunch did not adopt the exact surviving local-exec daemon identity/);
  assert.match(source, /Final graceful quit did not preserve the exact adopted local-exec daemon identity/);
  assert.match(source, /generationFingerprint/);
  assert.doesNotMatch(source, /reused an idle local-exec daemon instead of rotating/);
});
...
test("Windows packaged smoke uses authenticated loopback services and never embeds a private Tailnet dependency", async () => {
  ...
  assert.match(source, /gatewayPort = 1340/);
  assert.match(source, /listenLoopback\(gateway, gatewayPort\)/);
  assert.match(source, /gatewayBaseUrl: `http:\/\/127\.0\.0\.1:\$\{gatewayBoundPort\}`/);
  assert.match(source, /request\.url === "\/events"/);
  assert.match(source, /request\.url === "\/webauthn\/requests"/);
  assert.match(source, /request\.url === "\/local-exec\/requests"/);
  assert.match(source, /"\/webauthn\/responses", "\/local-exec\/responses"/);
  assert.match(source, /provider response batch used an unknown providerId/);
  assert.match(source, /an unbound provider startup batch must contain exactly one hello or ping/);
  assert.match(source, /candidates\.length !== 1/);
  assert.match(source, /only a WebAuthn startup ping may omit providerId/);
  assert.match(source, /activeLocalExecProviderIds\.delete\(providerId\)/);
  assert.match(source, /activeWebauthnProviderIds\.delete\(providerId\)/);
  assert.match(source, /activeWebauthnStreams\.delete\(providerId\)/);
  assert.match(source, /activeLocalExecStreams\.delete\(providerId\)/);
  assert.match(source, /gatewayState\.activeWebauthnStreams = activeWebauthnStreams\.size/);
  assert.match(source, /gatewayState\.activeLocalExecStreams = activeLocalExecStreams\.size/);
  assert.match(source, /audit\.rehello = priorHello !== undefined/);
  assert.match(source, /local-exec provider sent a conflicting hello retry/);
  assert.match(source, /webauthn provider sent a conflicting hello retry/);
  assert.match(source, /audit\.unboundStartupPing = true/);
  ...
  assert.match(source, /Persistent local-exec daemon did not complete distinct authenticated provider handshakes before and after relaunch/);
  ...
  assert.match(source, /command === "getForeverBoxStatus"/);
  assert.match(source, /getForeverBoxStatus must receive exactly one non-empty id/);
  assert.match(source, /\{ agentId: args\.id, state: "absent", vncUrl: null \}/);
  assert.match(source, /audit\.boundedOptionalCommand = true/);
  assert.match(source, /Optional signed-out coordinator command did not use its bounded response contract/);
  assert.match(source, /readJsonRequest\(request, 64 \* 1024\)/);
  assert.match(source, /request\.headers\.origin != null/);
  assert.match(source, /request\.windowsSmokeAudit\.invalid = true/);
  assert.match(source, /request\.invalid === true/);
  ...
  assert.match(source, /gateway\.once\("close", \(\) => \{/);
  assert.match(source, /disconnectProviderStreams\(\)/);
...
test("Windows smoke preserves the basic and --app entrypoints while defaulting CI to the full flow", async () => {
  ...
  assert.match(source, /export async function runWindowsSmokeCli/);
