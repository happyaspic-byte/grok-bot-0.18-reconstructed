...
test("Windows packaged smoke uses authenticated loopback services and never embeds a private Tailnet dependency", async () => {
  ...
  assert.match(source, /provider response batch used an unknown or inactive providerId/);
  assert.match(source, /only one local-exec startup hello may precede providerId binding/);
  assert.match(source, /unmatched\.length !== 1/);
  ...
  assert.match(source, /provider ping preceded hello/);
  ...
  assert.match(source, /First ready workspace did not complete fresh local-exec and WebAuthn provider handshakes/);
  assert.match(source, /Relaunched ready workspace did not complete fresh local-exec and WebAuthn provider handshakes/);
