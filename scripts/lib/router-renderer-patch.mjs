...
function RRouterCliProxy(){
  const[s,e]=de.useState(null),[t,n]=de.useState(""),[r,i]=de.useState(""),[o,l]=de.useState("chat-completions"),[c,d]=de.useState(""),[u,p]=de.useState(!1),[x,j]=de.useState(!1),[m,h]=de.useState(!1),g=v=>{e(v);n(v.baseUrl);i(v.model);l(v.protocol);p(v.allowRemoteHttps===!0);j(v.allowTailscaleHttp===!0)};
  de.useEffect(()=>{let v=!0;window.desktop.cliProxy.status().then(y=>{v&&g(y)});return()=>{v=!1}},[]);
  const f=async()=>{h(!0);try{g(await window.desktop.cliProxy.save({baseUrl:t,model:r,protocol:o,allowRemoteHttps:u,allowTailscaleHttp:x,...(c.trim()?{apiKey:c}:{})}));d("");await RRouterRefreshLocalWorkspace()}finally{h(!1)}},b=async()=>{h(!0);try{g(await window.desktop.cliProxy.remove());d("");await RRouterRefreshLocalWorkspace()}finally{h(!1)}},w=async()=>{h(!0);try{g(await window.desktop.cliProxy.status({testConnection:!0}))}finally{h(!1)}};
  return a.jsx(re,{title:"9Router connection",children:a.jsxs("div",{className:"sand-provider-usage-card",style:{display:"grid",gap:10},children:[
    a.jsxs("label",{children:[a.jsx("strong",{children:"Base URL"}),a.jsx("input",{"aria-label":"9Router Base URL",className:RRouterInputClass,disabled:m,onChange:v=>n(v.currentTarget.value),spellCheck:!1,type:"url",value:t}),a.jsx("small",{children:"Authenticated /v1 root only; default http://127.0.0.1:20128/v1. For this Tailscale server, use http://100.112.10.8:20128/v1. Never use /codex."})]}),
    a.jsxs("label",{children:[a.jsx("strong",{children:"Model ID"}),a.jsx("input",{"aria-label":"9Router model",className:RRouterInputClass,disabled:m,list:"sand-9router-models",onChange:v=>i(v.currentTarget.value),placeholder:"provider/model-id",spellCheck:!1,value:r}),a.jsx("datalist",{id:"sand-9router-models",children:s?.probe?.models?.map(v=>a.jsx("option",{value:v},v))}),a.jsx("small",{children:"If unknown, save the key first, run Test & load models, choose one, then save again. Manual entry stays available when /v1/models omits a model."})]}),
    a.jsxs("label",{children:[a.jsx("strong",{children:"API key"}),a.jsx("input",{"aria-label":"9Router API key",autoComplete:"new-password",className:RRouterInputClass,disabled:m,onChange:v=>d(v.currentTarget.value),placeholder:s?.configured?"Saved key (enter to replace)":"Proxy API key",type:"password",value:c}),a.jsx("small",{children:s?.configured?"A required proxy/client API key is saved. Leave this blank to keep it.":"Required. Use the 9Router proxy/client API key, not the management key."})]}),
    a.jsxs("label",{children:[a.jsx("strong",{children:"Protocol"}),a.jsxs("select",{"aria-label":"9Router protocol",className:RRouterInputClass,disabled:m,onChange:v=>l(v.currentTarget.value),value:o,children:[a.jsx("option",{value:"chat-completions",children:"Chat Completions (full tools)"}),a.jsx("option",{value:"responses",children:"Responses (not for Local Docker)"}),a.jsx("option",{value:"auto",children:"Auto (Chat first)"})]}),a.jsx("small",{children:"Use Chat Completions or Auto for the full Local Docker tool loop; explicit Responses is blocked for native-agent turns."})]}),
    a.jsxs("label",{style:{alignItems:"center",display:"flex",gap:8},children:[a.jsx("input",{checked:x,disabled:m,onChange:v=>j(v.currentTarget.checked),type:"checkbox"}),a.jsx("span",{children:"Allow HTTP over Tailscale (numeric IPs only; verify with tailscale ping)"})]}),
    a.jsxs("label",{style:{alignItems:"center",display:"flex",gap:8},children:[a.jsx("input",{checked:u,disabled:m,onChange:v=>p(v.currentTarget.checked),type:"checkbox"}),a.jsx("span",{children:"Allow a remote HTTPS endpoint (advanced; does not allow arbitrary HTTP)"})]}),
    a.jsx("small",{children:"Use the current stable 9Router release (v0.5.35 when reviewed); older builds have known authorization bypasses. Local Docker enables built-in agent, shell, file, browser, and computer tools; account-bound cloud features remain unavailable without sign-in."}),
    a.jsxs("div",{style:{display:"flex",gap:8,justifyContent:"flex-end"},children:[a.jsx(oe,{disabled:m||!s?.configured,onClick:b,shape:"rectangular",size:"sm",variant:"secondary",children:"Delete credential"}),a.jsx(oe,{disabled:m||!s?.configured,onClick:w,shape:"rectangular",size:"sm",variant:"secondary",children:"Test & load models"}),a.jsx(oe,{disabled:m||!t.trim()||(!s?.configured&&!c.trim()),onClick:f,shape:"rectangular",size:"sm",variant:"primary",children:m?"Saving…":"Save 9Router"})]}),
